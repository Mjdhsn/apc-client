...
// ------ presidential : analysisBy hook -------
const usePresidentialAnalysisBy = (location) => {
  ...
  const getAnalysisBy = () => {
    ...
    const apiUrl_firstPart =
      child === "Polling Unit" ? "/presidentialanalysis_polling_tab1" : 
      child === "Ward" ? "/presidentialanalysis_ward_tab1" : 
      child === "LGA" ? "/presidentialanalysis_lga_tab1":
      child === "State" ? "/presidentialanalysis_state_tab1" :
      child === "National" && "/presidentialanalysis_country_tab1"
...
// ------ senate : analysisBy hook -------
const useSenateAnalysisBy = (location) => {
  ...
  //hooks
  const [selectedLevel, setSelectedLevel] = useState(routeLevel);
  const [searchFields, setSearchFields] = useState({});
  const [searchLevel, setSearchLevel] = useState(routeLevel);
  const { token } = useToken()
  ...
  const getAnalysisBy = () => {
    // making decision for parent level
    const apiUrl_firstPart =
      child === "Polling Unit" ? "/senateanalysis_polling_tab1" : 
      child === "Ward" ? "/senateanalysis_ward_tab1" : 
      child === "LGA" ? "/senateanalysis_lga_tab1":
      child === "State" ? "/senateanalysis_state_tab1" :
      child === "District" ? "/senateanalysis_district_tab1" :
      child === "National" && "/senateanalysis_country_tab1"
...
// ------ REP : analysisBy hook -------
const useRepAnalysisBy = (location) => {
...
export { usePresidentialAnalysisBy, useSenateAnalysisBy, useRepAnalysisBy }